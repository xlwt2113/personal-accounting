...
| 图片识别（微信） | 图片中的**"交易单号"**字段值 | **必须逐字提取图片中的原始值**，如 `4200002564202505291234567890`（通常以 `42` 开头，长度约25位） |
...
> - 微信账单 → **"交易单号"** 字段，通常以 `42` 开头、长约25位数字（如 `4200003107202605200987917844`）。需要点击页面底部 **"更多 ▼"** 按钮展开后才能看到
> - 如果图片中确实无法识别到该字段，**必须向用户提示以下内容并拒绝入库**：
> > "图片信息不完整，缺少订单号/交易单号。支付宝账单请截取包含'订单号'的详情页；微信账单请先点击'更多'展开后再截图。"
